      ...
      <div style={{background:'#fff',borderRadius:10,border:'1px solid #e2e4e9',overflow:'visible',position:'relative'}}>
        ...
        :<table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
          ...
          <tbody>{filtered.map(q=>{const sc=STATUS[q.status]||STATUS.draft;
            ...
            const isSeen=seenQuotes.has(q.id);
            const showHighlight=needsAction&&!isSeen;
            const rowBg=isChecked?'#f0f7ff':showHighlight?'#fffbeb':'transparent';
            return(
            <tr key={q.id} style={{borderBottom:'1px solid #f3f4f6',background:rowBg,cursor:'pointer',borderLeft:showHighlight?'3px solid #d97706':'3px solid transparent'}} onMouseOver={e=>{if(!isChecked&&!showHighlight)e.currentTarget.style.background='#fafbfc';}} onMouseOut={e=>{e.currentTarget.style.background=rowBg;}}>
        ...
        <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:640,maxHeight:'90vh',overflowY:'auto'}}>
          {loadingDetail?<div style={{padding:40,textAlign:'center',color:'#8b919e'}}>Loading...</div>:detail&&<div>
            ...
            <div style={{display:'flex',gap:8,marginBottom:16}}>
              ...
              {(user.role==='reviewer'||user.role==='manager'||user.isAdmin)&&detail.status!=='approved'&&detail.status!=='sent'&&<button onClick={()=>window.open('/builder?quoteId='+detail.id,'_blank')} style={{flex:1,padding:'10px 16px',borderRadius:8,border:'none',background:'#d97706',color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer'}}>Edit Quote</button>}
            </div>
