  ...
  return (
    <div style={{maxWidth:1400,margin:'0 auto',padding:20}}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:16,alignItems:'center',flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex',gap:2,background:'#fff',padding:3,borderRadius:8,border:'1px solid #e2e4e9'}}>
          {['all','protein','bakery'].map(d=>(
            <button key={d} onClick={()=>setDivFilter(d)} style={{padding:'5px 10px',borderRadius:6,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:divFilter===d?'#003250':'transparent',color:divFilter===d?'#fff':'#8b919e',textTransform:'capitalize'}}>{d==='all'?'All Divisions':d}</button>))}
        </div>
        <div style={{display:'flex',gap:2,background:'#fff',padding:3,borderRadius:8,border:'1px solid #e2e4e9'}}>
          <button onClick={()=>setCoFilter('all')} style={{padding:'5px 10px',borderRadius:6,border:'none',cursor:'pointer',fontSize:10,fontWeight:600,background:coFilter==='all'?'#003250':'transparent',color:coFilter==='all'?'#fff':'#8b919e'}}>All Cos</button>
          {allCompanies.map(c=>(
            <button key={c.key} onClick={()=>setCoFilter(c.key)} style={{padding:'5px 10px',borderRadius:6,border:'none',cursor:'pointer',fontSize:10,fontWeight:700,background:coFilter===c.key?c.color:'transparent',color:coFilter===c.key?'#fff':c.color}}>{c.name}</button>))}
        </div>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{padding:'7px 14px',borderRadius:8,border:'1px solid #e2e4e9',fontSize:12,width:180}}/>
      ...
      <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e4e9',overflow:'hidden'}}>
        ...
        :<table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
          <thead><tr style={{background:'#f8f9fb'}}>
            {['Quote #','Status','Company','Customer','Amount','Created By','Date',''].map(h=>(
              <th key={h} style={{padding:'10px 14px',textAlign:h==='Amount'?'right':'left',fontSize:9,fontWeight:700,color:'#8b919e',borderBottom:'1px solid #e2e4e9'}}>{h}</th>))}
          </tr></thead>
        ...
        <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:640,maxHeight:'90vh',overflowY:'auto'}}>
          {loadingDetail?<div style={{padding:40,textAlign:'center',color:'#8b919e'}}>Loading...</div>:detail&&<div>
            ...
            <div style={{display:'flex',gap:8,marginBottom:16}}>
              <button onClick={()=>{setSelected(null);setDetail(null);window.location.href='/pdf';}} style={{flex:1,padding:'10px 16px',borderRadius:8,border:'1px solid #003250',background:'#fff',color:'#003250',fontSize:12,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                View PDF Proposal
              </button>
              <button onClick={()=>{setSelected(null);setDetail(null);window.location.href='/margin';}} style={{flex:1,padding:'10px 16px',borderRadius:8,border:'1px solid #059669',background:'#fff',color:'#059669',fontSize:12,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                View Margin Report
              </button>
            </div>
