...
import { dbToPrototypeCatalog, gP, cTot, fP, C, ROLES } from '@/lib/transform';
...
function CatPanel({ label, color, cat, sel, setSel, bg2 }) {
  ...
  function tog(sn, it, v) {
    const n = { ...sel };
    const ds = {};
    if (it.subs) it.subs.forEach(sub => { ds[sub.k] = (n[it.id]?.subs?.[sub.k]) || sub.choices[0]; });
    n[it.id] = { ...(n[it.id] || {}), on: v, q: n[it.id]?.q || (it.hq ? 1 : 0), op: n[it.id]?.op || (it.ops ? it.ops[0] : undefined), subs: { ...(n[it.id]?.subs || {}), ...ds } };
    setSel(n);
  }
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      {Object.entries(cat).map(([sn, sec]) => {
        const isO = openSec === sn; const items = sec.items || []; const c = items.filter(i => sel[i.id]?.on).length;
        return (
      ...
      <div style={{ width: 280, flexShrink: 0 }}>
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid ' + C.border, padding: 16, position: 'sticky', top: 64 }}>
          ...
          <div style={{ borderTop: '1px solid ' + C.border, paddingTop: 10, marginTop: 8 }}>
            ...
            <div style={{ fontSize: 9, fontWeight: 600, color: '#6b7085', background: '#f3f4f6', padding: '2px 8px', borderRadius: 10, display: 'inline-block', marginTop: 4 }}>Draft</div>
          </div>
        </div>
      </div>
      <div style={{ flex: 1 }}>
        {Object.keys(companies).map(k => {
          ...
          return <CatPanel key={k} label={co.name.toUpperCase()} color={co.color} cat={cats[k] || {}} sel={sels[k] || {}} setSel={s => setSels({ ...sels, [k]: s })} bg2={co.bg} />;
        })}
      </div>
    </div>
  );
