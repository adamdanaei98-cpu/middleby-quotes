...
export default function BuilderPage() {
  ...
  useEffect(() => {
    fetch('/api/users').then(r => r.ok ? r.json() : {}).then(d => setReviewers((d.users || []).filter(u => u.role === 'reviewer' || u.role === 'manager' || u.role === 'supervisor'))).catch(() => {});
  }, []);
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid ' + C.border, padding: 16, position: 'sticky', top: 64 }}>
          ...
          <div style={{ borderTop: '1px solid ' + C.border, paddingTop: 8, marginTop: 6 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontWeight: 800, color: C.navy, borderTop: '2px solid ' + C.navy, marginTop: 4, paddingTop: 6 }}>
              <span>Total</span><span>{fP(gt)}</span>
            </div>
          </div>
            ...
            {approval.status === 'draft' ? <div>
              <div style={{ marginBottom: 6 }}>
                ...
                <select value={selReviewer} onChange={e => setSelReviewer(e.target.value)} style={{ width: '100%', padding: '6px 10px', borderRadius: 6, border: '1px solid ' + C.border, fontSize: 11, boxSizing: 'border-box', background: '#fff' }}>
                  ...
                  {reviewers.map(u => <option key={u.id} value={u.id}>{u.name} — {u.role}</option>)}
                </select></div>
              <div style={{ marginBottom: 6 }}>
              ...
              <button onClick={async () => {
                ...
                try {
                  const activeCos = Object.keys(cats).filter(k => Object.values(sels[k] || {}).some(s => s && s.on));
                  const res = await fetch('/api/quotes', {
                    method: 'POST', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ mode, customerId: selectedCustomer || null, customerName: ci.name, contactName: ci.contact, contactEmail: ci.email, repName: ci.rep, purpose: ci.purpose, selections: sels, grandTotal: gt, companyKeys: activeCos, terms }),
                  });
                  if (res.ok) {
              ...
              }} disabled={!selReviewer} style={{ width: '100%', padding: 8, borderRadius: 8, border: 'none', fontSize: 12, fontWeight: 700, cursor: selReviewer ? 'pointer' : 'default', background: selReviewer ? '#d97706' : '#e5e7eb', color: selReviewer ? '#fff' : '#aaa' }}>Submit for Review</button>
              ...
              <button onClick={() => setApproval({ status: 'draft', reviewer: null, submittedAt: null, note: '' })} style={{ width: '100%', padding: 6, borderRadius: 6, border: '1px solid ' + C.border, fontSize: 10, fontWeight: 600, cursor: 'pointer', background: '#fff', color: C.muted }}>Recall Submission</button>
            </div> : <div>
              ...
              <button onClick={() => setApproval({ status: 'draft', reviewer: null, submittedAt: null, note: '' })} style={{ width: '100%', padding: 6, borderRadius: 6, border: '1px solid ' + C.border, fontSize: 10, fontWeight: 600, cursor: 'pointer', background: '#fff', color: C.muted }}>Reset to Draft</button>
            </div>}
          </div>
        </div>
      </div>
