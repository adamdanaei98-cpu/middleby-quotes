...
export default function BuilderPage() {
  const { user } = useAuth();
  const { cats, sels, setSels, companies, customers, loading, mode, setMode, ci, setCi, approval, setApproval, tots, gt } = useQuote();
        ...
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid ' + C.border, padding: 16, position: 'sticky', top: 64 }}>
          ...
          <div style={{ borderTop: '1px solid ' + C.border, paddingTop: 10, marginTop: 8 }}>
            ...
            {approval.status === 'draft' ? <div>
              ...
              <button onClick={() => { if (!selReviewer) return; setApproval({ status: 'pending', reviewer: selReviewer, submittedAt: new Date().toLocaleString(), note: submitNote }); setSubmitNote(''); }} disabled={!selReviewer} style={{ width: '100%', padding: 8, borderRadius: 8, border: 'none', fontSize: 12, fontWeight: 700, cursor: selReviewer ? 'pointer' : 'default', background: selReviewer ? '#d97706' : '#e5e7eb', color: selReviewer ? '#fff' : '#aaa' }}>Submit for Review</button>
