...
export default function AdminPage() {
  ...
  const { cats, setCats, companies, setCompanies, terms, setTerms, loading: qLoading } = useQuote();
  ...
  const cat = cats[co] || {};
  const updCo = (k, f, v) => { const n = { ...companies }; n[k] = { ...n[k], [f]: v }; setCompanies(n); };
    ...
    <div style={{ maxWidth: 1320, margin: '0 auto', padding: 20 }}>
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: 2, background: '#fff', padding: 4, borderRadius: 10, border: '1px solid ' + C.border }}>
          <TB id="profiles" l="Company Profiles" /><TB id="rates" l="Rates" /><TB id="catalog" l="Catalog" /><TB id="terms" l="Terms" /><TB id="users" l="Users" />
        </div>
      ...
      {tab==='profiles'&&<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:16}}>
        ...
        <div style={{background:'#fff',borderRadius:12,border:'2px dashed '+C.border,padding:20}}>
          ...
          <button onClick={()=>{if(!newCoName.trim())return;const key='co_'+Date.now();const n={...companies};n[key]={name:newCoName.trim(),color:newCoColor,bg:newCoColor+'12',desc:newCoDesc.trim(),execSummary:'',rates:{laborRate:30,pohr:2,markup:10,agentFee:5,commission:1.5,discount:0,freight:5000,install:15000}};setCompanies(n);setCats(p=>({...p,[key]:{}}));setNewCoName('');setNewCoDesc('');}} disabled={!newCoName.trim()} style={{padding:'8px 16px',background:newCoName.trim()?C.navy:'#ccc',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:newCoName.trim()?'pointer':'default'}}>Add Company</button>
        </div>
      </div>}
      ...
      {tab==='catalog'&&<div>
        ...
        {editItem&&<EditItemModal item={editItem} color={cd.color} onClose={()=>{setEditItem(null);setEditSec(null);}}
          onSave={updated=>{const nc=JSON.parse(JSON.stringify(cats));const s=nc[co][editSec];if(!s)return;const i=s.items.findIndex(x=>x.id===updated.id);if(i>=0)s.items[i]=updated;setCats(nc);setEditItem(null);setEditSec(null);}}
          onDelete={id=>{const nc=JSON.parse(JSON.stringify(cats));const s=nc[co][editSec];if(!s)return;s.items=s.items.filter(x=>x.id!==id);if(s.items.length===0)delete nc[co][editSec];setCats(nc);setEditItem(null);setEditSec(null);}}/>}
        {showAddItem&&<AddItemModal color={cd.color} sections={sectionNames} onClose={()=>setShowAddItem(false)}
          onSave={(item,secName)=>{const nc=JSON.parse(JSON.stringify(cats));if(!nc[co])nc[co]={};if(!nc[co][secName])nc[co][secName]={items:[]};nc[co][secName].items.push(item);setCats(nc);setShowAddItem(false);}}/>}
      </div>}
      ...
      {tab==='terms'&&<div style={{background:'#fff',borderRadius:12,border:'1px solid '+C.border,padding:24,maxWidth:900}}>
        ...
        <textarea value={terms} onChange={e=>setTerms(e.target.value)} rows={20} style={{...iS,lineHeight:1.7,fontFamily:'inherit',resize:'vertical'}}/>
