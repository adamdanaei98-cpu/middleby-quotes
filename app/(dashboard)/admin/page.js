...
const RO=[{v:'salesperson',l:'Salesperson'},{v:'reviewer',l:'Reviewer'},{v:'manager',l:'Manager'},{v:'supervisor',l:'Supervisor'}];
...
function AddItemModal({onClose,onSave,sections,color}){
  ...
  return(<div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:520}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}><div><div style={lS}>DESCRIPTION</div><input value={desc} onChange={e=>setDesc(e.target.value)} style={iS}/></div><div><div style={lS}>NOTE</div><input value={nt} onChange={e=>setNt(e.target.value)} style={iS}/></div></div>
...
export default function AdminPage() {
  ...
  const { cats, setCats, companies, setCompanies, customers, setCustomers, terms, setTerms, navLogo, setNavLogo, loading: qL } = useQuote();
  ...
  const isCorporate = user.role === 'supervisor' || (!user.primaryCompanyId && user.isAdmin);
  ...
  const saveCust = async (data, isNew) => {
    setErr('');try{const r=await fetch('/api/customers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});const d=await r.json();if(!r.ok){setErr(d.error);return;}
      if(isNew)setCustomers(p=>[...p,d.customer]);setShowModal(null);setOk(isNew?'Customer added':'Customer updated');setTimeout(()=>setOk(''),3000);}catch(e){setErr(e.message);}
  };
  ...
  const canEditUser = (u) => {
    if (isCorporate) return true; // corporate can edit anyone
    if (!user.primaryCompanyId) return false;
    return u.primaryCompanyId === user.primaryCompanyId; // company admin can only edit same company
  };
    ...
    <div style={{maxWidth:1320,margin:'0 auto',padding:20}}>
      ...
      {tab==='profiles'&&<div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(340px,1fr))',gap:16}}>
          {coKeys.map(k=>{const d=companies[k];return(
            <div key={k} style={{background:'#fff',borderRadius:12,border:'2px solid '+d.color,padding:20}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
                <div style={{position:'relative',cursor:'pointer'}} onClick={()=>pickFile('image/*',url=>updCo(k,'logo',url))}>
                  ...
                  <div style={{position:'absolute',bottom:-2,right:-2,width:14,height:14,borderRadius:7,background:d.color,border:'2px solid #fff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:7,color:'#fff'}}>{'\u270E'}</div></div>
              ...
              <div style={{marginBottom:10}}><div style={lS}>COMPANY NAME</div><input value={d.name} onChange={e=>updCo(k,'name',e.target.value)} style={iS}/></div>
              <div style={{display:'flex',gap:8,marginBottom:10}}><div style={{flex:1}}><div style={lS}>DESCRIPTION</div><input value={d.desc} onChange={e=>updCo(k,'desc',e.target.value)} style={iS}/></div><div style={{width:60}}><div style={lS}>COLOR</div><input type="color" value={d.color} onChange={e=>updCo(k,'color',e.target.value)} style={{width:'100%',height:30,borderRadius:6,border:'1px solid '+C.border,cursor:'pointer',padding:0}}/></div></div>
              <div style={{marginBottom:10}}><div style={lS}>EXEC SUMMARY</div><textarea value={d.execSummary||''} onChange={e=>updCo(k,'execSummary',e.target.value)} rows={3} style={{...iS,fontFamily:'inherit',resize:'vertical',lineHeight:1.5}}/></div>
              <div style={{marginBottom:10}}><div style={lS}>MACHINE IMAGE</div>
                {d.machineImg?<div style={{position:'relative'}}><img src={d.machineImg} style={{width:'100%',maxHeight:120,objectFit:'contain',background:'#f8f9fb',borderRadius:8}}/><button onClick={()=>updCo(k,'machineImg',null)} style={{position:'absolute',top:4,right:4,border:'none',background:'rgba(0,0,0,.5)',color:'#fff',borderRadius:10,width:20,height:20,cursor:'pointer',fontSize:10}}>{'\u00D7'}</button></div>
                :<button onClick={()=>pickFile('image/*',url=>updCo(k,'machineImg',url))} style={{padding:'8px 16px',border:'1px dashed '+d.color,background:'none',color:d.color,borderRadius:6,fontSize:11,cursor:'pointer'}}>Upload Machine Image</button>}</div>
            </div>);})}
          {/* Add company */}
          ...
          {/* Nav logo */}
          {isCorporate&&<div style={{background:'#fff',borderRadius:12,border:'2px solid '+C.navy,padding:20,display:'flex',alignItems:'center',gap:16}}>
            <div style={{width:80,height:50,borderRadius:8,background:'#002a3e',display:'flex',alignItems:'center',justifyContent:'center',padding:6,flexShrink:0}}>
              {navLogo?<img src={navLogo} style={{maxHeight:38,maxWidth:68,objectFit:'contain'}}/>:<span style={{color:'#fff',fontWeight:800,fontSize:14,fontStyle:'italic'}}>MIDDLEBY</span>}</div>
            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:C.navy}}>Nav Bar Logo</div><div style={{fontSize:10,color:C.muted,marginBottom:8}}>Replaces the text logo in the top bar.</div>
              <div style={{display:'flex',gap:8}}><button onClick={()=>pickFile('image/*',saveNavLogo)} style={{padding:'6px 14px',borderRadius:6,border:'1px solid '+C.navy,background:'transparent',color:C.navy,fontSize:11,fontWeight:600,cursor:'pointer'}}>Upload Logo</button>
                {navLogo&&<button onClick={()=>saveNavLogo('')} style={{padding:'6px 14px',borderRadius:6,border:'1px solid #fecaca',background:'#fef2f2',color:'#dc2626',fontSize:11,cursor:'pointer'}}>Remove</button>}</div></div>
          </div>}
        </div>
      </div>}
      ...
      {tab==='rates'&&cd&&<div style={{background:'#fff',borderRadius:12,border:'2px solid '+cd.color,padding:20,maxWidth:500}}>
        ...
        {[['laborRate','Labor Rate ($)'],['pohr','POHR (X)'],['markup','Markup %'],['agentFee','Agent Fee %'],['commission','Commission %'],['freight','Freight ($)'],['install','Install ($)']].map(([rk,l])=>(
          <div key={rk} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid #f3f4f6'}}>
            ...
            <input type="number" step="0.1" value={(cd.rates&&cd.rates[rk])||0} onChange={e=>{const r={...(cd.rates||{}),[rk]:parseFloat(e.target.value)||0};updCo(co,'rates',r);}} style={{width:80,padding:'4px 8px',borderRadius:5,border:'1px solid '+C.border,fontSize:11,textAlign:'right'}}/>
          </div>))}
      </div>}
      ...
      {tab==='catalog'&&<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
          <div><span style={{fontSize:16,fontWeight:700,color:cd.color}}>{cd.name} Catalog</span></div>
          <button onClick={()=>setShowAddItem(true)} style={{padding:'6px 14px',background:cd.color,color:'#fff',border:'none',borderRadius:6,fontSize:11,fontWeight:700,cursor:'pointer'}}>+ Add Item</button></div>
          ...
          <div key={sn} style={{marginBottom:8,borderRadius:10,overflow:'hidden',border:'1px solid '+C.border,background:'#fff'}}>
            ...
            {isOpen&&<table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}><thead><tr style={{background:'#f9fafb'}}>{['Item','Fixed $','Options','Material $','Labor Hrs'].map(x=><th key={x} style={{padding:'6px 12px',textAlign:x==='Item'?'left':'right',fontSize:8,fontWeight:700,color:C.muted}}>{x}</th>)}</tr></thead>
              <tbody>{items.map(it=>{const hasOps=it.ops&&it.ops.length>0;return(
                <tr key={it.id} onClick={()=>{setEditItem(it);setEditSec(sn);}} style={rowHov} onMouseOver={e=>e.currentTarget.style.background=cd.color+'06'} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
      ...
      {tab==='terms'&&<div style={{background:'#fff',borderRadius:12,border:'1px solid '+C.border,padding:24,maxWidth:900}}>
        ...
        <textarea value={terms} onChange={e=>updTerms(e.target.value)} rows={20} style={{...iS,lineHeight:1.7,fontFamily:'inherit',resize:'vertical'}}/>
      ...
      {/* ═══ CUSTOMERS ═══ - row click to edit */}
      {tab==='customers'&&<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <h2 style={{fontSize:18,fontWeight:800,color:'#003250',margin:0}}>Customers ({customers.length})</h2>
          <button onClick={()=>{setModalData({name:'',plant:'',address:'',contact:'',email:'',phone:'',rep:'',industry:''});setShowModal('addCust');}} style={{padding:'8px 18px',background:'#003250',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer'}}>+ Add Customer</button></div>
        <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e4e9',overflow:'hidden'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{background:'#f8f9fb'}}>
            ...
            <tr key={c.id} onClick={()=>{setModalData({...c});setShowModal('editCust');}} style={rowHov} onMouseOver={e=>e.currentTarget.style.background='#fafbfc'} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
      ...
      {/* ═══ USERS ═══ - grouped by company, row click to edit */}
      {tab==='users'&&<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <h2 style={{fontSize:18,fontWeight:800,color:'#003250',margin:0}}>Users ({users.length})</h2>
          <button onClick={()=>{setModalData({name:'',email:'',password:'',role:'salesperson',isAdmin:false,primaryCompanyId:''});setShowModal('addUser');}} style={{padding:'8px 18px',background:'#003250',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer'}}>+ Add User</button></div>
        {/* Corporate users */}
        {(()=>{const corp=users.filter(u=>!u.primaryCompanyId);if(!corp.length)return null;return(
          <div style={{marginBottom:20}}>
            <div style={{fontSize:13,fontWeight:700,color:C.navy,marginBottom:8,display:'flex',alignItems:'center',gap:8}}><div style={{width:4,height:16,borderRadius:2,background:C.navy}}/> Corporate ({corp.length})</div>
            <div style={{background:'#fff',borderRadius:10,border:'1px solid #e2e4e9',overflow:'hidden'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{background:'#f8f9fb'}}>
                {['Name','Email','Role','Admin','Status'].map(h=><th key={h} style={{padding:'8px 14px',textAlign:'left',fontSize:9,fontWeight:700,color:'#8b919e',borderBottom:'1px solid #e2e4e9'}}>{h}</th>)}
              </tr></thead><tbody>{corp.map(u=>(
                <tr key={u.id} onClick={()=>{if(canEditUser(u)){setModalData({...u,password:''});setShowModal('editUser');}}} style={{...rowHov,cursor:canEditUser(u)?'pointer':'default',opacity:canEditUser(u)?1:.7}} onMouseOver={e=>{if(canEditUser(u))e.currentTarget.style.background='#fafbfc';}} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
                  <td style={{padding:'8px 14px',fontWeight:600,color:'#003250'}}>{u.name}</td>
                  <td style={{padding:'8px 14px',color:'#555'}}>{u.email}</td>
                  <td style={{padding:'8px 14px'}}><span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:10,background:u.role==='supervisor'?'#dbeafe':'#f3f4f6',color:u.role==='supervisor'?'#2563eb':'#6b7085'}}>{RO.find(r=>r.v===u.role)?.l}</span></td>
                  <td style={{padding:'8px 14px'}}>{u.isAdmin&&<span style={{fontSize:10,fontWeight:600,color:'#E12C3E',background:'#fef2f2',padding:'2px 8px',borderRadius:10}}>Admin</span>}</td>
                  <td style={{padding:'8px 14px'}}><span style={{fontSize:10,fontWeight:600,color:u.active?'#16a34a':'#dc2626'}}>{u.active?'Active':'Inactive'}</span></td>
                </tr>))}</tbody></table></div>
          </div>);})()}
        {/* Per-company users */}
        {dbCos.map(dco=>{const coUsers=users.filter(u=>u.primaryCompanyId===dco.id);if(!coUsers.length)return null;return(
          <div key={dco.id} style={{marginBottom:20}}>
            <div style={{fontSize:13,fontWeight:700,color:dco.color,marginBottom:8,display:'flex',alignItems:'center',gap:8}}><div style={{width:4,height:16,borderRadius:2,background:dco.color}}/> {dco.name} ({coUsers.length})</div>
            <div style={{background:'#fff',borderRadius:10,border:'1px solid #e2e4e9',overflow:'hidden'}}>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr style={{background:'#f8f9fb'}}>
                {['Name','Email','Role','Admin','Status'].map(h=><th key={h} style={{padding:'8px 14px',textAlign:'left',fontSize:9,fontWeight:700,color:'#8b919e',borderBottom:'1px solid #e2e4e9'}}>{h}</th>)}
              </tr></thead><tbody>{coUsers.map(u=>(
                <tr key={u.id} onClick={()=>{if(canEditUser(u)){setModalData({...u,password:''});setShowModal('editUser');}}} style={{...rowHov,cursor:canEditUser(u)?'pointer':'default',opacity:canEditUser(u)?1:.7}} onMouseOver={e=>{if(canEditUser(u))e.currentTarget.style.background='#fafbfc';}} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
                  <td style={{padding:'8px 14px',fontWeight:600,color:'#003250'}}>{u.name}</td>
                  <td style={{padding:'8px 14px',color:'#555'}}>{u.email}</td>
                  <td style={{padding:'8px 14px'}}><span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:10,background:u.role==='manager'?'#dcfce7':u.role==='reviewer'?'#fef9c3':'#f3f4f6',color:u.role==='manager'?'#16a34a':u.role==='reviewer'?'#a16207':'#6b7085'}}>{RO.find(r=>r.v===u.role)?.l}</span></td>
                  <td style={{padding:'8px 14px'}}>{u.isAdmin&&<span style={{fontSize:10,fontWeight:600,color:'#E12C3E',background:'#fef2f2',padding:'2px 8px',borderRadius:10}}>Admin</span>}</td>
                  <td style={{padding:'8px 14px'}}><span style={{fontSize:10,fontWeight:600,color:u.active?'#16a34a':'#dc2626'}}>{u.active?'Active':'Inactive'}</span></td>
                </tr>))}</tbody></table></div>
          </div>);})}
