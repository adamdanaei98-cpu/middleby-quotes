    ...
    <div style={{maxWidth:1320,margin:'0 auto',padding:20}}>
      ...
      {tab==='catalog'&&<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
          ...
          {canEditCo(co)&&<button onClick={()=>setShowAddItem(true)} style={{padding:'6px 14px',background:cd.color,color:'#fff',border:'none',borderRadius:6,fontSize:11,fontWeight:700,cursor:'pointer'}}>+ Add Item</button>}</div>
      ...
      {tab==='customers'&&<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          ...
          <button onClick={()=>{setModalData({name:'',plant:'',address:'',contact:'',email:'',phone:'',keywords:[],notes:'',companyId:co!=='__corporate'?cd?.id||'':''});setShowModal('addCust');}} style={{padding:'8px 18px',background:'#003250',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer'}}>+ Add Customer</button></div>
        {(()=>{
      ...
      {tab==='users'&&<div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          ...
          {(isCorporate||(userCoKey&&co===userCoKey))&&<button onClick={()=>{setModalData({name:'',email:'',password:'',role:'salesperson',isAdmin:false,primaryCompanyId:co==='__corporate'?'':cd?.id||''});setShowModal('addUser');}} style={{padding:'8px 18px',background:'#003250',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer'}}>+ Add User</button>}</div>
        {(()=>{
