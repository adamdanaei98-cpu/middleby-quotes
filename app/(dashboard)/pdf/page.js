...
export default function PDFPage() {
  const { cats, sels, companies, ci, mode, terms, loading } = useQuote();
  ...
  return (
    <div style={{ padding: 20, background: '#e5e7eb', minHeight: 'calc(100vh - 56px)' }}>
      ...
      <div className="pdf-doc pdf-page" style={ps}>
        ...
        <div style={{ padding: '10px 40px 30px' }}>
          ...
          <div style={{ display: 'flex', gap: 20, marginBottom: 20, padding: '14px 18px', background: '#f8f9fb', borderRadius: 8, border: '1px solid ' + C.border }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 8, fontWeight: 700, color: C.muted, marginBottom: 3 }}>PREPARED FOR</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.navy }}>{ci.name || '—'}</div>
              {ci.contact && <div style={{ fontSize: 11, color: '#555' }}>{ci.contact}</div>}
            </div>
            {ci.rep && <div style={{ flex: 1 }}><div style={{ fontSize: 8, fontWeight: 700, color: C.muted, marginBottom: 3 }}>YOUR REPRESENTATIVE</div><div style={{ fontSize: 14, fontWeight: 700, color: C.navy }}>{ci.rep}</div></div>}
          </div>
          {activeCos.length === 0 && <div style={{ textAlign: 'center', padding: 40, color: C.muted }}>Select items in the Builder to generate the proposal.</div>}
          {activeCos.length > 0 && mode === 'bundle' && <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: C.navy, marginBottom: 12 }}>The Complete Line Solution</div>
            {activeCos.map(k => { const co = companies[k]; return (
              <div key={k} style={{ display: 'flex', gap: 12, padding: '10px 14px', marginBottom: 8, borderRadius: 8, border: '1px solid ' + co.color + '30', background: co.bg }}>
                <div style={{ width: 8, height: 36, borderRadius: 4, background: co.color, flexShrink: 0 }} />
                <div style={{ flex: 1 }}><div style={{ fontSize: 12, fontWeight: 700, color: co.color }}>{co.name}</div><div style={{ fontSize: 9, color: co.color, opacity: .7 }}>{co.desc}</div>{co.execSummary && <div style={{ fontSize: 9, lineHeight: 1.5, color: '#555', marginTop: 3 }}>{co.execSummary}</div>}</div>
              </div>
            ); })}
          </div>}
          {ci.purpose && <div style={{ fontSize: 10, fontStyle: 'italic', color: '#555', padding: '8px 12px', background: '#f9fafb', borderRadius: 6, marginTop: 12 }}>{ci.purpose}</div>}
        </div>
        {stripe}
      </div>
      ...
      {/* Summary */}
      {activeCos.length > 0 && <div className="pdf-doc pdf-page" style={ps}>
        {hdr()}
        <div style={{ padding: '10px 40px 30px' }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: C.navy, marginBottom: 14 }}>Price Summary</div>
          {activeCos.map(k => { const t = cTot(cats[k] || {}, sels[k] || {}); return <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #eee' }}><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 10, height: 10, borderRadius: 2, background: companies[k]?.color }} /><span style={{ fontSize: 12, fontWeight: 700, color: companies[k]?.color }}>{companies[k]?.name}</span></div><span style={{ fontSize: 12, fontWeight: 700 }}>{fP(t.g)}</span></div>; })}
          ...
          {terms && <div style={{ marginTop: 32, paddingTop: 16, borderTop: '1px solid #ddd' }}>
            ...
            {terms.split('\n').map((line, i) => { if (!line.trim()) return <div key={i} style={{ height: 6 }} />; const isH = /^\d+\./.test(line.trim()); return <div key={i} style={{ fontSize: 9, lineHeight: 1.6, color: isH ? '#333' : '#555', fontWeight: isH ? 600 : 400 }}>{line}</div>; })}
          </div>}
        </div>
        {stripe}
      </div>}
