    ...
    <div style={{padding:20,background:'#e5e7eb',minHeight:'calc(100vh - 56px)'}}>
      ...
      <div className="pdf-doc" style={ps}>
        ...
        <div style={{padding:'24px 40px'}}>
          ...
          {/* Bundle intro - only show when multiple companies */}
          {activeCos.length>1&&mode==='bundle'&&<div style={{marginBottom:20}}>
            ...
            return(
              <div key={k} className="pdf-page" style={{marginTop:28,paddingTop:16,borderTop:'3px solid '+co.color}}>
                ...
                {items.map((x,i)=>{
                  ...
                  if(pr===0&&!x.it.hq)return null; // skip $0 items without qty
                  const descFirst = x.it.desc ? x.it.desc.split('\n').filter(l=>l.trim())[0]?.replace(/^[•\-]\s*/,'').trim() : '';
                  return(<Fragment key={i}>
                    ...
                    {descFirst&&<div style={{fontSize:9,color:'#888',paddingLeft:8,fontStyle:'italic',marginTop:1}}>{descFirst.substring(0,120)}</div>}
                  </Fragment>);})}
