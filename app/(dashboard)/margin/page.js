...
import { gP, cTot, itemDN, fP, fD, C } from '@/lib/transform';
    ...
    <div style={{ padding: 20, background: '#f0f2f5', minHeight: 'calc(100vh - 56px)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: C.navy }}>Margin Calculator</div>
            <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{ci.name}{ci.name ? ' • ' : ''}Rev. {ci.revision}</div>
          </div>
          <div style={{ fontSize: 9, color: '#dc2626', fontWeight: 700, background: '#fef2f2', padding: '4px 12px', borderRadius: 6 }}>INTERNAL ONLY</div>
        </div>
              ...
              <div key={coName} style={{ marginBottom: 24, borderRadius: 10, overflow: 'hidden', border: '1px solid ' + C.border, background: '#fff' }}>
                <div style={{ background: grp.color, padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                    <div style={{ textAlign: 'right' }}><div style={{ fontSize: 8, color: 'rgba(255,255,255,.5)' }}>COST</div><div style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>{fD(grp.totals.cost)}</div></div>
