...
function MarginContent() {
  ...
  const handleDownload = async () => {
    setGenerating(true);
    try {
      const { generateMarginPDF } = await import('@/lib/pdfgen');
      const doc = generateMarginPDF({ cats, sels, companies, ci, gP, itemDN, cTot });
      doc.save(`Margin_${ci.proposalNumber || 'Report'}_Rev${ci.revision || 1}.pdf`);
