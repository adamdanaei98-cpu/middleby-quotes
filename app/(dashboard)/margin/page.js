...
import { gP, itemDN, fP, fD, C } from '@/lib/transform';

export default function MarginPage() {
  const { cats, sels, companies, ci, loading } = useQuote();
  ...
  const mEq=tList-t3rd; const manC=tMat+tLC+tPO; const mRev=tList-manC;
  ...
  const kv=(l,v,bold,clr)=><div style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #f3f4f6'}}><span style={{fontSize:12,fontWeight:bold?700:400}}>{l}</span><span style={{fontSize:12,fontWeight:600,color:clr||C.text}}>{v}</span></div>;
  ...
  return (
    <div style={{padding:20,background:'#f0f2f5',minHeight:'calc(100vh - 56px)'}}>
      <div className="no-print" style={{textAlign:'center',marginBottom:16}}>
        <button onClick={()=>window.print()} style={{padding:'8px 24px',borderRadius:8,border:'none',background:C.green,fontSize:13,fontWeight:600,color:'#fff',cursor:'pointer'}}>Print Margin Report</button></div>
        ...
        {has&&<>
              ...
              <div key={coName} style={{marginBottom:24,borderRadius:10,overflow:'hidden',border:'1px solid '+C.border,background:'#fff'}}>
                <div style={{background:grp.color,padding:'14px 20px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div style={{display:'flex',alignItems:'center',gap:12}}>
                    ...
                    <span style={{fontSize:9,fontWeight:700,color:'rgba(255,255,255,.6)',background:'rgba(255,255,255,.15)',padding:'3px 10px',borderRadius:10}}>{itemCount} items • {grp.secs.length} sections</span></div>
                  <div style={{display:'flex',alignItems:'center',gap:20}}>
                    ...
                    <div style={{textAlign:'right'}}><div style={{fontSize:8,fontWeight:700,color:'rgba(255,255,255,.5)'}}>MARGIN</div><div style={{fontSize:13,fontWeight:700,color:'#fff'}}>{fD(grp.totals.mg)}</div></div>
                    <div style={{background:'rgba(255,255,255,.2)',padding:'6px 14px',borderRadius:6,fontSize:15,fontWeight:800,color:'#fff',minWidth:48,textAlign:'center'}}>{coMp}%</div></div></div>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                  ...
                  <tbody>
                    {grp.secs.map((sec,si)=>[
                      <tr key={'hdr-'+si} style={{background:grp.color+'08'}}>
                        <td colSpan={7} style={{padding:'8px 12px',borderTop:si>0?'2px solid '+C.border:'none'}}>
                          <div style={{display:'flex',alignItems:'center',gap:8}}><div style={{width:3,height:14,borderRadius:2,background:grp.color}}/><span style={{fontSize:11,fontWeight:700,color:grp.color}}>{sec.sn}</span></div></td>
                    ...
                    <tr style={{background:grp.color+'10'}}>
                      <td style={{padding:'12px',fontWeight:800,color:grp.color,borderTop:'3px solid '+grp.color,fontSize:12}}>{coName} Total</td>
                      <td style={{padding:'12px',borderTop:'3px solid '+grp.color}}/>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:700,borderTop:'3px solid '+grp.color}}>{fD(grp.totals.mat)}</td>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:700,borderTop:'3px solid '+grp.color}}>{grp.totals.lh.toFixed(1)}</td>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:700,borderTop:'3px solid '+grp.color}}>{fD(grp.totals.lc)}</td>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:700,borderTop:'3px solid '+grp.color}}>{fD(grp.totals.po)}</td>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:700,borderTop:'3px solid '+grp.color}}>{fD(grp.totals.cost)}</td>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:800,borderTop:'3px solid '+grp.color,color:grp.color,fontSize:12}}>{fP(grp.totals.lp)}</td>
                      <td style={{padding:'12px',textAlign:'right',fontWeight:800,borderTop:'3px solid '+grp.color,color:coMp>=40?'#16a34a':'#ca8a04'}}>{fD(grp.totals.mg)}</td>
                      <td style={{padding:'12px',textAlign:'right',borderTop:'3px solid '+grp.color}}>{pill(coMp)}</td></tr>
                  </tbody></table></div>);})}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginTop:8}}>
            <div style={{background:'#fff',borderRadius:10,border:'1px solid '+C.border,padding:18}}>
              ...
              {kv('Middleby Equipment',fD(mEq))}{kv('3rd Party Components',fD(t3rd))}
              {kv('Total Package',fD(tList),true,C.navy)}{kv('Total Contract',fD(tList),true)}</div>
            <div style={{background:'#fff',borderRadius:10,border:'1px solid '+C.border,padding:18}}>
              ...
              {kv('Material Cost',fD(tMat))}{kv('Labor Hours',tLH.toFixed(1)+' hrs')}
              {kv('Labor Cost',fD(tLC))}{kv('POHR',fD(tPO))}</div>
            <div style={{background:'#fff',borderRadius:10,border:'1px solid '+C.border,padding:18}}>
              ...
              {kv('Equipment + 3rd Party',fD(tList))}
              {kv('Manufacturing Cost',fD(-manC),false,'#dc2626')}
              {kv('Margin Revenue',fD(mRev),true,mPct>=40?'#16a34a':'#ca8a04')}
