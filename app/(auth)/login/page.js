      ...
      <div style={{ width: 420, background: '#fff', borderRadius: 16, padding: 40, boxShadow: '0 20px 60px rgba(0,0,0,.3)' }}>
        ...
        {demos.length > 0 && <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid #e2e4e9' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {demos.map(a => <div key={a.email} onClick={() => { setEmail(a.email); setPassword(''); setError(''); }} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid #e2e4e9', cursor: 'pointer', fontSize: 10, lineHeight: 1.4 }}><div style={{ fontWeight: 700, color: brand.navColor }}>{a.name}</div><div style={{ color: '#8b919e' }}>{a.role} • {a.level}</div></div>)}
          </div>
        </div>}
      </div>
    </div>
