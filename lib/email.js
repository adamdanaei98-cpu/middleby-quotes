...
<div style="margin-bottom:16px">
  ${(quote.companyKeys || []).map(k => `<span style="display:inline-block;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:600;color:#003250;background:#eef4ff;border:1px solid #d0dff0;margin-right:4px">${k}</span>`).join('')}
  <span style="display:inline-block;padding:3px 10px;border-radius:12px;font-size:10px;font-weight:600;color:#333;background:#f3f4f6;border:1px solid #e2e4e9">${quote.mode === 'bundle' ? 'Bundle' : 'Individual'}</span>
</div>`;
      ...
      <div style="padding:24px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px">
          <div>
            ...
            <div style="font-size:11px;color:#8b919e">Rev. ${quote.revision || 1} • ${quote.mode || 'bundle'} • ${new Date().toLocaleDateString()}</div>
          </div>
    ...
    html: emailWrapper('Reviewed', '#2563eb', `
      <div style="padding:24px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px">
          <div>
            <div style="font-size:22px;font-weight:800;color:#003250;margin-bottom:2px">${quote.quoteNumber}</div>
            <div style="font-size:11px;color:#8b919e">Rev. ${quote.revision || 1} • ${quote.mode || 'bundle'}</div>
          </div>
