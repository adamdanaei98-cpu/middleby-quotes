...
const NAV = [{ id: '/builder', l: 'Builder' }, { id: '/quotes', l: 'Quotes' }];
  ...
  return (
    <div className="no-print" style={{ background: bg, padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 8px rgba(0,0,0,.2)' }}>
      ...
      <div style={{ display: 'flex', gap: 2 }}>
        {NAV.map(n => (
          <button key={n.id} onClick={() => router.push(n.id)} style={{ padding: '6px 16px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, background: path === n.id ? '#fff' : 'transparent', color: path === n.id ? bg : 'rgba(255,255,255,.6)' }}>{n.l}</button>
        ))}
        {canAdmin && (
