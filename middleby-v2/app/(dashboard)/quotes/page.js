...
export default function QuotesPage() {
  ...
  const getActions = () => {
    ...
    const isReviewer = user.role === 'reviewer'; // company level only
    const isManager = user.role === 'manager'; // company level only
    // Supervisor is view-only on quote workflow
    const actions = [];
  ...
  return (
    <div style={{maxWidth:1400,margin:'0 auto',padding:20}}>
      ...
      <p style={{fontSize:12,color:'#8b919e',marginBottom:20}}>{user.role==='supervisor'?'Corporate oversight — all quotes (view only)':user.role==='manager'?'Quotes awaiting your final approval':user.role==='reviewer'?'Quotes submitted for your review':'Your quotes'}</p>
      ...
      <div style={{display:'flex',gap:8,marginBottom:16,alignItems:'center',flexWrap:'wrap'}}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{padding:'7px 14px',borderRadius:8,border:'1px solid #e2e4e9',fontSize:12,width:220}}/>
      ...
      <div style={{background:'#fff',borderRadius:12,border:'1px solid #e2e4e9',overflow:'hidden'}}>
        ...
        :<table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
          <thead><tr style={{background:'#f8f9fb'}}>
            {['Quote #','Status','Customer','Amount','Created By','Date',''].map(h=>(
          ...
          <tbody>{filtered.map(q=>{const sc=STATUS[q.status]||STATUS.draft;return(
        ...
        <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:640,maxHeight:'90vh',overflowY:'auto'}}>
          {loadingDetail?<div style={{padding:40,textAlign:'center',color:'#8b919e'}}>Loading...</div>:detail&&<div>
            ...
            {(()=>{
              ...
              if (actions.length === 0 && user.role === 'supervisor') {
                return <div style={{borderTop:'2px solid #e2e4e9',paddingTop:16,marginTop:8}}>
                  <div style={{fontSize:11,color:'#8b919e',fontStyle:'italic'}}>Corporate oversight — view only. Quote actions are handled by company-level staff.</div>
                </div>;
              }
