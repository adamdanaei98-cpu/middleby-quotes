...
export function canAccessAdmin(user) {
  if (!user) return false;
  if (user.role === 'supervisor' || user.role === 'it') return true;
...
export function canEditCompanyData(user, companyId) {
  ...
  if (user.role === 'it' && !user.primaryCompanyId) return true; // corporate IT
  if (user.role === 'supervisor' && !user.isAdmin) return false;
  if (user.isAdmin || user.role === 'reviewer' || user.role === 'it') {
...
export function canManageUsers(user) {
  if (!user) return false;
  if (isCorporateAdmin(user)) return true;
  if (user.role === 'it') return true; // IT always manages users
  if (user.isAdmin) return true;
  return false;
}
